...
def append_texts_from_blend(blend_path, existing_names, flag_keywords, flagged_results):
    ...
    # Attempt to load .blend; if it fails, let the caller handle the exception
    ...
    for txt in bpy.data.texts:
        if txt.name not in existing_names:
            ...
            if matched:
                flagged_results.append((new_name, matched))
...
# -------------------------------------------------------------------
# 3) REPORT GENERATION HELPER (now includes skipped files)
# -------------------------------------------------------------------
def generate_and_show_flag_report(flagged_results, flag_keywords, skipped_files):
    ...
    Write a formatted report of:
      ...
      - All flagged text blocks (and which keywords matched).
    ...
    if flagged_results:
        for name, matches in flagged_results:
            report.write(f">> {name}\n")
...
# -------------------------------------------------------------------
# 4) MAIN OPERATOR (tracks skipped_files)
# -------------------------------------------------------------------
    ...
    def execute(self, context):
        ...
        for fname in blend_files:
            ...
            try:
                ...
                # Record the skip for the report
        ...
        # 4) Show the very first appended text block if available
        ...
        # 6) Final summary in the Info area
